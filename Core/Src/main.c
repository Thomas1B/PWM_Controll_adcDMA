...
//		float pulseWidth1 = map(potVoltage, 0.0, 3.3, 0.0, 2000); // Map voltage to duty cycle percentage
//		uint16_t correctedValue = gammaCorrect(adcValue, 2.2f); // Apply gamma correction to the ADC value
//		float pulseWidth2 = map(correctedValue, 0.0, 4095.0, 0.0, 2000); // Map gamma-corrected value to duty cycle percentage

//		__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_2, pulseWidth1); // Update PWM duty cycle based on ADC reading using gamma correction
//		__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_3, pulseWidth2); // Update PWM duty cycle based on ADC reading
